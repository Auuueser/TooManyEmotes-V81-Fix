...
namespace TooManyEmotes.Patches
{
    ...
    public static class ThirdPersonEmoteController
    {
        ...
        private static MethodInfo changeAudioListenerToObjectMethod;
        ...
        public static void OnStopCustomEmoteLocal()
        {
            ...
            if (StartOfRound.Instance.activeCamera != gameplayCamera)
                StartOfRound.Instance.SwitchCamera(gameplayCamera);
            if (!IsAudioListenerParentedTo(gameplayCamera.gameObject))
                CallChangeAudioListenerToObject(gameplayCamera.gameObject);
        ...
        {
            if (!localPlayerController || !gameObject)
                return;

            if (firstPersonEmotesEnabled && (!localPlayerController.gameplayCamera || gameObject != localPlayerController.gameplayCamera.gameObject))
                return;

            if (!TryEnsureActiveAudioListener(localPlayerController, gameObject))
                return;

            MethodInfo method = GetChangeAudioListenerToObjectMethod(localPlayerController);
            if (method != null)
            {
                try
                {
                ...
                {
                    LogWarningVerbose("Falling back after ChangeAudioListenerToObject failed.\n" + e);
                }
            }

            MoveAudioListenerToObject(localPlayerController, gameObject);
        }


        private static MethodInfo GetChangeAudioListenerToObjectMethod(PlayerControllerB playerController)
        {
            if (!playerController)
                return null;

            if (changeAudioListenerToObjectMethod == null)
                changeAudioListenerToObjectMethod = playerController.GetType().GetMethod("ChangeAudioListenerToObject", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            return changeAudioListenerToObjectMethod;
        }


        private static bool IsAudioListenerParentedTo(GameObject gameObject)
        {
            return localPlayerController && localPlayerController.activeAudioListener && gameObject && localPlayerController.activeAudioListener.transform.parent == gameObject.transform;
        }


        private static void MoveAudioListenerToObject(PlayerControllerB playerController, GameObject gameObject)
        {
            if (!playerController || !playerController.activeAudioListener || !gameObject)
                return;

            playerController.activeAudioListener.transform.SetParent(gameObject.transform);
            playerController.activeAudioListener.transform.localEulerAngles = Vector3.zero;
            playerController.activeAudioListener.transform.localPosition = Vector3.zero;
            if (StartOfRound.Instance != null)
                StartOfRound.Instance.audioListener = playerController.activeAudioListener;
            if (IngamePlayerSettings.Instance != null && IngamePlayerSettings.Instance.flipCamera)
                playerController.activeAudioListener.transform.Rotate(0f, 180f, 0f, Space.Self);
        }


        private static bool TryEnsureActiveAudioListener(PlayerControllerB playerController, GameObject targetObject)
        {
            if (!playerController || !targetObject)
                return false;

            if (!playerController.activeAudioListener)
            {
                AudioListener audioListener = StartOfRound.Instance?.audioListener;
                if (!audioListener && playerController.gameplayCamera)
                    audioListener = playerController.gameplayCamera.GetComponentInChildren<AudioListener>(true) ?? playerController.gameplayCamera.GetComponent<AudioListener>();
                if (!audioListener)
                    audioListener = playerController.GetComponentInChildren<AudioListener>(true);
                if (!audioListener)
                {
                    GameObject audioListenerObject = new GameObject("TooManyEmotesAudioListener");
                    audioListenerObject.transform.SetParent(targetObject.transform);
                    audioListenerObject.transform.localEulerAngles = Vector3.zero;
                    audioListenerObject.transform.localPosition = Vector3.zero;
                    audioListener = audioListenerObject.AddComponent<AudioListener>();
                }

                playerController.activeAudioListener = audioListener;
            }

            playerController.activeAudioListener.enabled = true;
            if (!playerController.activeAudioReverbFilter)
                playerController.activeAudioReverbFilter = playerController.activeAudioListener.GetComponent<AudioReverbFilter>() ?? playerController.activeAudioListener.gameObject.AddComponent<AudioReverbFilter>();
            if (StartOfRound.Instance != null && !StartOfRound.Instance.audioListener)
                StartOfRound.Instance.audioListener = playerController.activeAudioListener;
